...
class ThreeLayerConvNet(object):
  ...
  def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
    ...
    self.params['W2'] = np.random.normal(scale=weight_scale,
                                         size=(num_filters*out_size*out_size,
                                               hidden_dim))
  ...
  def loss(self, X, y=None):
    ...
    # variable.                                                                #
    ############################################################################
    a1, a1_cache = conv_relu_pool_forward(X, self.params['W1'], 
                                          self.params['b1'], 
    ...
    ############################################################################
    loss, da3 = softmax_loss(a3, y)
    loss += 0.5 * self.reg * np.sum(self.params['W1'] ** 2)
    loss += 0.5 * self.reg * np.sum(self.params['W2'] ** 2)

    da3, dw3, db3 = affine_backward(da3, a3_cache)
    da2, dw2, db2 = affine_relu_backward(da3, a2_cache)
    da1, dw1, db1 = conv_relu_pool_backward(da2, a1_cache)
