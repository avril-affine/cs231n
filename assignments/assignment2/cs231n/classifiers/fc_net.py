...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    da2, dw2, db2 = affine_backward(da2, a2_cache)
    da2 = relu_backward(da2, a1r_cache)
    da1, dw1, db1 = affine_backward(da2, a1_cache)
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for i, hidden_dim in enumerate(hidden_dims, start=1):
        ...
        if self.use_batchnorm:
            gamma_name = 'gamma{0}'.format(i)
            beta_name = 'beta{0}'.format(i)
        ...
        self.params[W_name] = np.random.normal(scale=weight_scale,
                                               size=(prev_dim, hidden_dim))
        self.params[b_name] = np.zeros(hidden_dim)
        prev_dim = hidden_dim
    W_name = 'W{0}'.format(self.num_layers)
    b_name = 'b{0}'.format(self.num_layers)
    self.params[W_name] = np.random.normal(scale=weight_scale,
  ...
  def loss(self, X, y=None):
    ...
    for i in xrange(1, self.num_layers):
        ...
        if self.use_batchnorm:
            gamma_name = 'gamma{0}'.format(i)
            beta_name = 'beta{0}'.format(i)
            out, batch_cache = batchnorm_forward(out,
                                                 self.params[gamma_name],
                                                 self.params[beta_name],
                                                 self.bn_params[i-1])
            bn_cache.append(batch_cache)

        out, rel_cache = relu_forward(out)
        cache.append((aff_cache, rel_cache))
        if self.use_dropout:
    ...
    W_name = 'W{0}'.format(self.num_layers)
    b_name = 'b{0}'.format(self.num_layers)
    aff_out, aff_cache = affine_forward(out,
                                        self.params[W_name],
                                        self.params[b_name])
    scores = aff_out
    ...
    i = self.num_layers - 1
    for aff_cache, rel_cache in reversed(cache):
        W_name = 'W{0}'.format(i)
        b_name = 'b{0}'.format(i)

        if self.use_dropout:
            d_cache = dropout_cache.pop()
            da = dropout_backward(da, d_cache)

        da = relu_backward(da, rel_cache)

        if self.use_batchnorm:
            gamma_name = 'gamma{0}'.format(i)
            beta_name = 'beta{0}'.format(i)
            batch_cache = bn_cache.pop()
            da, dgamma, dbeta = batchnorm_backward(da, batch_cache)
            grads[gamma_name] = dgamma
            grads[beta_name] = dbeta

        da, dw, db = affine_backward(da, aff_cache)
